...
for($i=0; $i<count($array); $i++) {
    if($i != count($array)-1) {
        if(intval($array[$i]['billroth_v']) != 0) {
            echo "<tr>";
            echo '<td>'. $array[$i]['pzn'] . '</td>';
            echo '<td>'. $array[$i]['Bezeichnung'] . '</td>';
            echo '<td>'. $array[$i]['Menge'] . '</td>';
            echo '<td>'. $array[$i]['Einheit'] . '</td>';
            echo '<td>'. $array[$i]['billroth_k'] . '</td>';
            echo '<td>'. $array[$i]['billroth_v'] . '</td>';
            echo "</tr>";
        }
    } else {
        echo '<tr class="footer">';
        echo '<td>Summe</td><td></td><td></td><td></td><td>'.$array[$i]['billroth_k'].'</td><td>'.$array[$i]['billroth_v'].'</td>';
        echo "</tr>";
    }
}
?>
