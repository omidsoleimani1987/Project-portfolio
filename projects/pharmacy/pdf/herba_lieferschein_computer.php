...
for($i=0; $i<count($array); $i++) {
    if($i != count($array)-1) {
        if(intval($array[$i]['herba_v']) != 0) {
            echo "<tr>";
            echo '<td>'. $array[$i]['pzn'] . '</td>';
            echo '<td>'. $array[$i]['Bezeichnung'] . '</td>';
            echo '<td>'. $array[$i]['Menge'] . '</td>';
            echo '<td>'. $array[$i]['Einheit'] . '</td>';
            echo '<td>'. $array[$i]['herba_k'] . '</td>';
            echo '<td>'. $array[$i]['herba_v'] . '</td>';
            echo '<td>'. $array[$i]['herba_prozent'] . '</td>';
            echo "</tr>";
        }
    } else {
        echo '<tr class="footer">';
        echo '<td></td><td></td><td></td><td></td><td></td><td>'.$array[$i]['herba_k'].'</td><td>'.$array[$i]['herba_v'].'</td><td></td>';
        echo "</tr>";
    }
}
?>
...
echo "<table>";
echo "<tr>";
echo "<th>ID</th><th>PZN</th><th>Bezeichnung</th><th>Menge</th><th>Einheit</th><th>Bestellt</th><th>Verkauf</th><th>Prozent</th>";
echo "</tr>";
for($i=0; $i<count($array); $i++) {
    if($i != count($array)-1) {
        echo "<tr>";
        ...
        echo "<tr>";
        echo '<td></td><td></td><td></td><td></td><td></td><td>'.$array[$i]['herba_k'].'</td><td>'.$array[$i]['herba_v'].'</td>';
        echo "</tr>";
    }
}
echo "</table>";